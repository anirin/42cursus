...
{
	printf("philo [%d] address left[%d]:%p right[%d]:%p\n", data.philo.index, data.philo.index,
		data.philo.left_fork, (data.philo.index + 1) % data.setting.number, data.philo.right_fork);
}
...
void philo_sleep(t_data data)
{
	...
	print_state_now(data, SLEEPING);
	// print_state(data, SLEEPING);
	usleep(data.setting.time_to_sleep * 1000);
